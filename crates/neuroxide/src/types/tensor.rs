...
impl<T: TensorElement> Tensor<T> {
    ...
    pub fn set_op(&mut self, op: Arc<Mutex<dyn Operation<T>>>) {
        self.op = Some(op);
    }
    ...
    pub fn get_op(&self) -> Option<Arc<Mutex<dyn Operation<T>>>> {
        self.op.clone()
    }
    ...
    T: std::fmt::Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Tensor")
            .field("data", &self.data.get_values())
            .field("shape", &self.data.get_shape())
