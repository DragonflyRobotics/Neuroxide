...
use crate::ops::op_generic::{Operation, Ops};
    ...
    T: TensorElement,
{
    fn forward(inputs: &Vec<&Tensor<T>>) -> Tensor<T> {
        ...
        match inputs[0].device {
            Device::CPU => {
                let mut a =
                    ArrayD::<T>::from_shape_vec(IxDyn(&inputs[0].shape), inputs[0].data.clone())
                        .unwrap();
                let mut b =
                    ArrayD::<T>::from_shape_vec(IxDyn(&inputs[1].shape), inputs[1].data.clone())
                        .unwrap();
                ...
                if inputs[0].shape.len() == 1 && inputs[1].shape.len() == 1 {
                    ...
                    unsafe {
                        ...
                        matmul(
                            1 as i32,
                            1 as i32,
                            inputs[0].shape[0] as i32,
                            inputs[0].cuda_ptr.unwrap(),
                            inputs[1].cuda_ptr.unwrap(),
                            &mut ptr_to_data,
                        );
                        cuda_ptr = Some(ptr_to_data);
                    }
                } else if inputs[0].shape.len() == 2 && inputs[1].shape.len() == 2 {
                    assert!(inputs[0].shape[1] == inputs[1].shape[0], "Matrix multiplication requires the second dimension of the first matrix to match the first dimension of the second matrix");
                    result = vec![T::default(); inputs[0].shape[0] * inputs[1].shape[1]];
                    #[cfg(feature = "cuda")]
                    unsafe {
                        ...
                        matmul(
                            inputs[0].shape[0] as i32,
                            inputs[1].shape[1] as i32,
                            inputs[0].shape[1] as i32,
                            inputs[0].cuda_ptr.unwrap(),
                            inputs[1].cuda_ptr.unwrap(),
                            &mut ptr_to_data,
                        );
                    ...
                    for (b_og, b_broad) in inputs[1].shape[0..inputs[1].shape.len() - 2]
                        .iter()
                    ...
                    let batch_dims: i32 = shape[0..shape.len() - 2]
                        .to_vec()
                        .iter()
                        .product::<usize>()
                        .try_into()
                    ...
                    result =
                        vec![
                            T::default();
                            batch_dims as usize * shape[shape.len() - 2] * shape[shape.len() - 1]
                        ];
                    ...
                    panic!("Matrix multiplication only supported n@n, (m, n)@(n, m) or (..., m, n)@(...,n, m) tensors");
                }
            }
        }
    ...
    fn backward(inputs: &Vec<&Tensor<T>>, grad: Option<&Tensor<T>>, device: Device) -> Tensor<T> {
        ...
        let grad_index = inputs
            .iter()
            .position(|&x| x.id == grad.unwrap().id)
            .unwrap();
        let mut b_arr = ArrayD::<T>::from_shape_vec(
            IxDyn(&inputs[1 - grad_index].shape),
            inputs[1 - grad_index].data.clone(),
        )
        .unwrap();
        let mut b_shape = inputs[1 - grad_index].shape.clone();
        let mut cuda_ptr: Option<*mut f32> = None;
        ...
        if device == Device::CPU {
            ...
            if b_arr_option.is_none() {
                b_arr = b_arr
                    .to_shape(IxDyn(&mul_shapes[1 - grad_index]))
                    .unwrap()
                    .to_owned();
            } else {
                b_arr = b_arr_option.unwrap().to_owned();
            }
            b_shape = mul_shapes[1 - grad_index].clone();
            ...
            if b_shape.len() == 1 {
                ...
                unsafe {
                    // println!("Transposing shape: {:?}", b_shape);
                    let mut data: f32 = 0.0;
                    let mut ptr_to_data: *mut f32 = &mut data;
                    transpose(
                        b_shape[0] as i32,
                        b_shape[1] as i32,
                        b_arr.as_mut_ptr() as *mut f32,
                        &mut ptr_to_data,
                    );
                    b_t_shape = vec![b_shape[1], b_shape[0]];
                    cuda_ptr = Some(ptr_to_data);
                ...
                let d: i32 = b_shape[0..b_shape.len() - 2]
                    ...
                    .try_into()
                    .unwrap();
                let m: i32 = b_shape[b_shape.len() - 2].try_into().unwrap();
                let n: i32 = b_shape[b_shape.len() - 1].try_into().unwrap();
                unsafe {
                    let mut data: f32 = 0.0;
                    let mut ptr_to_data: *mut f32 = &mut data;
                    transpose_b(
                        ...
                        n,
                        1,
                        inputs[1 - grad_index].cuda_ptr.unwrap(),
                        &mut ptr_to_data,
                    );
                    let mut permuted_axes: Vec<usize> = (0..b_shape.len()).collect();
                    permuted_axes.swap(b_shape.len() - 2, b_shape.len() - 1); // Swap the last two axes
                    b_t_shape = permuted_axes.iter().map(|&x| b_shape[x]).collect();
                    cuda_ptr = Some(ptr_to_data);
                    b_t = ArrayD::<T>::zeros(IxDyn(&b_t_shape));
                    println!("Finished Transpose yielding shape: {:?}", b_t_shape);
                }
            } else {
                panic!("Matrix multiplication grad only supported for <=3D tensors");
            }
