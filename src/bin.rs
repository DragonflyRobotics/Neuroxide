...
fn main() {
    let tensor_db = Arc::new(Mutex::new(TensorDB::new()));
    let x = Tensor::new(vec![5.0], vec![1], Device::CPU, true);
    let c1c = Tensor::new(vec![15.0], vec![1], Device::CPU, false);
    let c2c = Tensor::new(vec![6.0], vec![1], Device::CPU, false);
