...
inline bool checkCUDASuccess(cudaError_t err)
{
    if (err != cudaSuccess)
    {
...
inline void copyDataToCUDAMemory(float* d_var, const float* h_var, size_t size)
{
    checkCUDASuccess(cudaMemcpy(d_var, h_var, size, cudaMemcpyHostToDevice));
}

inline void copyDataToHostMemory(float* h_var, float* d_var, size_t size)
{
