...
fn forward() {
    ...
    for i in 0..result.data.len() {
        assert!(relative_eq!(result.data[i], x.data[i].sin(), epsilon = f32::EPSILON));
    }
    assert_eq!(result.shape, x.shape);
}
...
fn forward_cuda() {
    ...
    let mut result = SinOp::forward(&vec![&x]);
    result.cpu();
...
fn backward() {
    ...
    for i in 0..result.data.len() {
        assert!(relative_eq!(grad.get(&x.id).unwrap().data[i], x.data[i].cos(), epsilon = f32::EPSILON));
    }
}
...
fn downcasts_u32() {
    ...
    let x = Tensor::<u32>::new(&db, vec![0, 1, 2, 3, 4], vec![4], Device::CPU, false);
    let _ = SinOp::forward(&vec![&x]);
}
...
fn downcasts_u64() {
    ...
    let x = Tensor::<u64>::new(&db, vec![0, 1, 2, 3, 4], vec![4], Device::CPU, false);
    let _ = SinOp::forward(&vec![&x]);
}
...
fn downcasts_u128() {
    ...
    let x = Tensor::<u128>::new(&db, vec![0, 1, 2, 3, 4], vec![4], Device::CPU, false);
    let _ = SinOp::forward(&vec![&x]);
}
