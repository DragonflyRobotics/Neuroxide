...
fn new () {
    ...
    let c1c = Tensor::new(db.clone(), vec![15.0, 2.0, 3.1], vec![3, 1], Device::CPU, false);
...
fn clear_graph() {
    let db = Arc::new(RwLock::new(TensorDB::new(DTypes::F32)));
    let mut x = Tensor::new(db.clone(), vec![5.0], vec![1], Device::CPU, true);
...
fn dtypes() {
    ...
    let x = Tensor::new(db.clone(), vec![5], vec![1], Device::CPU, true);
